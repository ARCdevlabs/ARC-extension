...
if module.AutodeskData():
    ...
    t = Transaction (doc, "Create filter: Start Level Offset of Beam")
    t.Start()
    ...
    collector_floor = FilteredElementCollector(doc).OfCategory(BuiltInCategory.OST_StructuralFraming).WhereElementIsNotElementType()
    collector_floor.ToElements()
    list_floorS = []
    # list_type = all_type(FloorType, BuiltInCategory.OST_StructuralFraming)
    ...
    for i in list_floorS:
        try:
            # parameter_floor_height = module.get_builtin_parameter_by_name(i, BuiltInParameter.STRUCTURAL_BEAM_END0_ELEVATION)
            value_height_offset = module.get_builtin_parameter_by_name(i, BuiltInParameter.STRUCTURAL_BEAM_END0_ELEVATION).AsValueString()
    ...
    class MyForm(Form):
        def __init__(self):
            ...
            self.textBox_1.Text = "Frame_start level offset_e_"
            ...
            self.typeNameLabel.Text = "level offset"
        ...
        def on_print_button_click(self, sender, e):
            ...
            sym_name_param = ElementId(BuiltInParameter.STRUCTURAL_BEAM_END0_ELEVATION)
            list_floorS_tem = []
            count = []  
            for i in list_floorS:
            ...
            for i_height in checked_items:
                for floor in list_floorS_tem:
                    ...
                    try:
                        ...
                        floor_height = module.get_builtin_parameter_by_name(floor, BuiltInParameter.STRUCTURAL_BEAM_END0_ELEVATION).AsValueString()
                        try:
