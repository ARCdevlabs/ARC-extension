...
from Autodesk.Revit.DB import LogicalAndFilter, FilteredElementCollector, Transaction, BuiltInParameter, BuiltInCategory, WallType, CeilingType, FloorType, ElementId, FilterRule, ParameterFilterElement, ElementParameterFilter, ParameterFilterRuleFactory
...
def all_type (ofClass, string_OST):
    ...
    return all_OST_type
list_type = all_type(WallType, BuiltInCategory.OST_Walls)
name = []
for i in list_type:
...
class MyForm(Form):
    ...
    def on_print_button_click(self, sender, e):
        ...
        category.Add(ElementId(BuiltInCategory.OST_Walls))
            ...
            for wall in list_tem:
                try:
                    ...
                    if i_name == wall_name:
                            ...
                            rules.Add(ParameterFilterRuleFactory.CreateEqualsRule(sym_name_param, wall_name, True))
                            for rule in rules:
