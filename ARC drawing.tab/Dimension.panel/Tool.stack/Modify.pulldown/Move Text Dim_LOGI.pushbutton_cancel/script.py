...
if nances.AutodeskData():
    ...
    for element in Ele:
        try:
            ...
            for seg in segments:
                text_ori = seg.Origin
                all_seg.append(text_ori)
            farthest_points = find_farthest_points(all_seg)
            return_point = find_midpoint (farthest_points[0],farthest_points[1])
            for seg in segments:
                text_ori = seg.Origin
            ...
            for seg_ben_phai in sorted_phai:
                ...
                if len (sorted_phai) > 1:
                    kich_co_phai = kich_co_phai + view_scale + xac_dinh_kich_co_chu(current_view, value_phai*304.8) /he_so
                else:
                    kich_co_phai = kich_co_phai + xac_dinh_kich_co_chu(current_view, value_phai*304.8) /he_so
                ...
                kich_co_phai = kich_co_phai + 3* xac_dinh_kich_co_chu(current_view, value_phai*304.8)/he_so
                add_suffix_to_dimension (seg_ben_phai)
            try:
            ...
            for seg_ben_trai in sorted_trai:
                ...
                if len (sorted_trai) > 1:
                    kich_co_trai = kich_co_trai + view_scale +  xac_dinh_kich_co_chu(current_view, value*304.8)/he_so
                else: 
                    kich_co_trai = kich_co_trai +  xac_dinh_kich_co_chu(current_view, value*304.8)/he_so
                move_trai = move_point_along_vector(vi_tri_trai, vector, -(kich_co_trai)/304.8)
                seg_ben_trai.TextPosition = move_trai
                kich_co_trai = kich_co_trai +  3* xac_dinh_kich_co_chu(current_view, value*304.8)/he_so
        except:
            pass
    t.Commit()
