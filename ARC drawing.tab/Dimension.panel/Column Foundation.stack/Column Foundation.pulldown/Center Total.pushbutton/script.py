...
if not Ele:
    ...
    sys.exit()
try:
    # dialog = Autodesk.Revit.UI.TaskDialog("ARC: Thông báo")
    # dialog.MainContent = "Khi hiện bảng thông báo thì phải bấm Don't cut geometry"
    # dialog.TitleAutoPrefix = False
    # dialog.Show() 

    t_1 = Transaction(doc,"Prepare for dim")
    # options_1 = t_1.GetFailureHandlingOptions()
    # options_1.SetClearAfterRollback(False)
    # options_1.SetForcedModalHandling(True)
    # options_1.SetDelayedMiniWarnings(False)
    # t_1.SetFailureHandlingOptions()
    t_1.Start()
    list_can_not_dim = []         
    for column in Ele:
        ...
        if has_modified_geo == False:
            ...
            if len(list_can_not_dim) > 0 and len(Ele) < 2 :
                dialog = Autodesk.Revit.UI.TaskDialog("ARC")
                dialog.MainContent = "Thử với 2 hoặc hơn 2 đối tượng cần dim"
                dialog.TitleAutoPrefix = False
                dialog.Show() 
            else:
        ...
        try:
            ...
            for face in faces:
                try:
                    ...
                    # face_to_plane_X = Plane.CreateByNormalAndOrigin(face_normal, face_origin)
...
# Tao detail line de kiem chung
            ...
            pass
t.Commit()
def uncut_geometry(idoc, list_bi_cut, list_cut):
    uncut_geometry = Autodesk.Revit.DB.SolidSolidCutUtils.RemoveCutBetweenSolids(idoc, list_cut, list_bi_cut)
    return


# t_4 = Transaction(doc,"Dimension column")
# t_4.Start()
# for tung_ele in list_can_not_dim:
#     uncut_geometry(doc,list_comprehension[0],tung_ele )
# t_4.Commit()
