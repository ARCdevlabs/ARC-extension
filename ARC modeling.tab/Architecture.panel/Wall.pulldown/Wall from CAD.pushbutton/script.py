...
try:
    if module.AutodeskData():
        ...
        if str(current_applocale) == "日本語 / Japanese (ja)":
            messeage = "DWGインポートインスタンスを選択し、[OK]をクリックして続行してください"
        else:
            messeage = "Please select the DWG importinstance, Click OK to continue"
        module.message_box(messeage)

        with forms.WarningBar(title="Select the DWG importinstance"):
