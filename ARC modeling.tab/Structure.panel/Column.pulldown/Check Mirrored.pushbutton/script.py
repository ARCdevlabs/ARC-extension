...
ele = module.get_all_elements_of_OST(doc, BuiltInCategory.OST_StructuralColumns)
lstpoint = []
listok=[]
listnotok= []
...
try:
    if len(ele) == 0:
        Alert('Do not have column in the model',title="Warning",header= "Something wrong")
        Alert("Please select again", exit= True)
    else:
        # Place your code below this line
        t = Transaction(doc, "Check Mirrored Column")
        t.Start()
        for i in ele:
            try:
